    ...
    try (TimeSpatialIndexCreator timeSpatialIndexCreator =
      ...
      try {
        timeSpatialIndexCreator.set(
            VehiclesResponseParserTestHelper.getResourceAsPath(
                "data/tier4-digitalzensoapi/vehicles/vehicles_20241119000000.json"));
      } catch (Exception e) {
        // 検証
        // 正常に終了すること
        fail("正常に終了すること", e);
  ...
  void set_required() {
    ...
    try (TimeSpatialIndexCreator timeSpatialIndexCreator =
      ...
      try {
        timeSpatialIndexCreator.set(
            VehiclesResponseParserTestHelper.getResourceAsPath(
                VehiclesResponseParserTestHelper.REQUIERD_FILE_NAME));
      } catch (Exception e) {
        // 検証
        // updatedAtのないデータはスキップされて正常に終了すること
        fail("正常に終了すること", e);
      }
    }
  }
  ...
  void set_illegal_location_spatial_id() {
    ...
    try (TimeSpatialIndexCreator timeSpatialIndexCreator =
      ...
      try {
        timeSpatialIndexCreator.set(
            VehiclesResponseParserTestHelper.getResourceAsPath(
                VehiclesResponseParserTestHelper.ILLEGAL_LOCATION_SPATIAL_ID_FILE_NAME));
      } catch (Exception e) {
        // 検証
  ...
  void set_illegal_location_axispot() {
    ...
    try (TimeSpatialIndexCreator timeSpatialIndexCreator =
      ...
      try {
        timeSpatialIndexCreator.set(
            VehiclesResponseParserTestHelper.getResourceAsPath(
                VehiclesResponseParserTestHelper.ILLEGAL_LOCATION_AXISPOT_FILE_NAME));
      } catch (Exception e) {
        // 検証
        // 範囲外の位置データはスキップされて正常に終了すること
        fail("正常に終了すること", e);
