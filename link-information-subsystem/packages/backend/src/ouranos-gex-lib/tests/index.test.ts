...
describe("Space", () => {
  it("should be able to create a Space from lng/lat/alt", () => {
    ...
    expect(space.zfxyStr).toBe("/25/10/16777216/16777216");
    expect(space.tilehash).toBe("4111111111111111111115151");
  });

  it("works", () => {
    const space = new Space("1/0/0/0");
    expect(space.zfxy).toBe({ z: 1, f: 0, x: 0, y: 0 });
    expect(space.zfxyStr).toBe("/1/0/0/0");
    expect(space.tilehash).toBe("1");
    expect(space.center).toBe({
      alt: 8388608,
      lat: 66.51326044311186,
      lng: -90,
    });

    expect(space.up(1).zfxy).toBe({ z: 1, f: 1, x: 0, y: 0 });
    expect(space.up(1).tilehash).toBe("5");
    expect(space.down(1).zfxy).toBe({ z: 1, f: -1, x: 0, y: 0 });
    expect(space.down(1).tilehash).toBe("-5");
  });
  ...
  describe("parent", () => {
    it("returns the correct parent coordinates with no arguments", () => {
      ...
      expect(space.parent().zfxy).toBe({ z: 24, f: 0, x: 14901652, y: 6606228 });
    });
    it("returns the correct parent coordinates at a specified zoom level", () => {
      const space = new Space("25/0/29803304/13212456");
      expect(space.parent(23).zfxy).toBe({ z: 23, f: 0, x: 7450826, y: 3303114 });
      expect(space.parent(22).zfxy).toBe({ z: 22, f: 0, x: 3725413, y: 1651557 });
      expect(space.parent(14).zfxy).toBe({ z: 14, f: 0, x: 14552, y: 6451 });
      expect(space.parent(0).zfxy).toBe({ z: 0, f: 0, x: 0, y: 0 });
    });
  });
  ...
  describe("surroundings", () => {
    it("works", () => {
      ...
      expect(zfxyStrs.length).toBe(26);
      expect(zfxyStrs).toBe([
  ...
  describe("vertices3d", () => {
    it("works", () => {
      ...
      expect(vertices3d.length).toBe(8);
      expect(vertices3d).toBe([
...
const testPolygons: { [key: string]: Polygon } = {
  SIMPLE: {
    coordinates: [
      [
        [139.7404337636242, 35.676005522333085],
        [139.7404337636242, 35.67374730758844],
        [139.7427432114959, 35.67374730758844],
        [139.7427432114959, 35.676005522333085],
        [139.7404337636242, 35.676005522333085],
      ],
    ],
    type: "Polygon",
  ...
  DETAILED: {
    coordinates: [
      [
        [139.73132532824928, 35.66657408663923],
        [139.7308657092451, 35.66613900145083],
        [139.73099702895985, 35.66602897953335],
        [139.73108320752306, 35.666103994493525],
        [139.7309847177366, 35.666184010373385],
        [139.7312986539318, 35.666487403189706],
        [139.7314894778924, 35.66635737783834],
        [139.7310544813364, 35.665987304525416],
        [139.73134174321268, 35.665758925093],
        [139.73141561055252, 35.66583560731111],
        [139.73118169730958, 35.66599897352954],
        [139.73124940903864, 35.66605398452789],
        [139.73146280357656, 35.66589061842207],
        [139.73154487839815, 35.665950630499836],
        [139.73131506889678, 35.66610232749561],
        [139.73142586990656, 35.666199013341924],
        [139.73170287243033, 35.66614900343582],
        [139.7315941232913, 35.66622901927059],
        [139.73171107991362, 35.66633403993197],
        [139.73132532824928, 35.66657408663923],
      ],
    ],
    type: "Polygon",
...
describe("spacesForGeometry", () => {
  it("returns the correct spaces for a simple polygon", () => {
    ...
    expect(spaces.map((s) => s.zfxy)).toBe([
