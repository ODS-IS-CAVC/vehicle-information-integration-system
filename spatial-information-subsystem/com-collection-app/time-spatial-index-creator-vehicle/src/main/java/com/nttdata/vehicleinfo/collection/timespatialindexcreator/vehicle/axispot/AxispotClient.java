...
public class AxispotClient implements AutoCloseable {
  ...
  /**
   * 指定された車両情報をAxispotに格納する。
   *
   * @param vehicleInformation 車両情報
   * @throws IllegalStateException 既にクローズ済み
   */
  public void set(VehicleInformation vehicleInformation) {
    if (this.geotemp == null) {
      throw new IllegalStateException("既にクローズ済み");
    }
    MovingObjectKey key = getMovingObjectKey(vehicleInformation);
    MovingObjectValue value = getMovingObjectValue(vehicleInformation);
  ...
  static MovingObjectKey getMovingObjectKey(VehicleInformation information) {
    MovingObjectKey key =
        new MovingObjectKey(
            information.time(),
  ...
  static MovingObjectValue getMovingObjectValue(VehicleInformation information) {
    Coordinate locCoord =
