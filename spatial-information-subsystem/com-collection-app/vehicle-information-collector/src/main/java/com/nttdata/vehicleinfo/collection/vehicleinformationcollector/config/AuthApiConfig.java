 ...
 * AxispotConfig
 *
 * <p>設定ファイルの「vehicle.auth-api」に対応する。
 */
@ConfigurationProperties(prefix = "vehicle.auth-api")
public record AuthApiConfig(Login login) {

  public record Login(
      @NotEmpty(message = "環境変数「VEHICLE_AUTH_API_LOGIN_KEY」にて設定すること") String key, Request request) {
